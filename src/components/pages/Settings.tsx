...
export function SettingsPage() {
  ...
  return (
    <div className="flex-1 flex flex-col px-8 py-6 overflow-y-auto">
      ...
      >
        <h2 className={`text-2xl font-bold mb-2 ${theme === 'light' ? 'text-[#1E293B]' : ''}`}>设置</h2>
        <p className={`text-sm mb-8 ${theme === 'light' ? 'text-[#64748B]' : 'text-text-secondary'}`}>管理你的 OpenClaw</p>
      </motion.div>
      ...
      <motion.div
        ...
        className="mb-6"
      >
        <h3 className={`text-lg font-medium mb-4 flex items-center gap-2 ${theme === 'light' ? 'text-[#1E293B]' : ''}`}>
          <Play size={18} />
          服务状态
        </h3>
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              <div className={`w-3 h-3 rounded-full ${
                ...
                serviceStatus === 'stopped' ? (theme === 'light' ? 'bg-[#94A3B8]' : 'bg-text-secondary') : 'bg-status-warning animate-pulse'
              }`} />
              <span className={`font-medium ${theme === 'light' ? 'text-[#1E293B]' : ''}`}>
                {serviceStatus === 'running' ? '运行中' : 
                 serviceStatus === 'stopped' ? '已停止' : '检测中...'}
              </span>
              {versionInfo.installed && versionInfo.version && (
                <span className={`text-sm ml-2 ${theme === 'light' ? 'text-[#64748B]' : 'text-text-secondary'}`}>
      ...
      >
        ...
        <div className={`rounded-xl p-4 border ${
          ...
          <div className="flex items-center justify-between">
            <span className={theme === 'light' ? 'text-[#1E293B]' : ''}>主题</span>
            <div className="flex gap-2">
              <button
      ...
      >
        ...
        <div className={`rounded-xl p-4 border ${
          ...
          <div className="flex items-center justify-between">
            <span className={theme === 'light' ? 'text-[#1E293B]' : ''}>界面语言</span>
            <div className="flex gap-2">
              <button
      ...
      >
        ...
        <div className={`rounded-xl p-4 border ${
          ...
          <div className="flex items-center justify-between">
            <div>
              <div className={`font-medium ${theme === 'light' ? 'text-[#1E293B]' : ''}`}>导入/导出配置</div>
              <div className={`text-sm ${theme === 'light' ? 'text-[#64748B]' : 'text-text-secondary'}`}>备份或恢复你的设置</div>
            </div>
            <div className="flex gap-2">
